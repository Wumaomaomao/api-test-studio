...
export default function AutoTestModule({ projects, apis, styles }) {
  ...
  const handleAddTestCase = () => {
    ...
    setTestCases([...testCases, newCaseWithId]);
    localStorage.setItem('testCases', JSON.stringify([...testCases, newCaseWithId]));
  ...
  const handleDeleteTestCase = (caseId) => {
    const updatedCases = testCases.filter(c => c.id !== caseId);
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            // 保存测试用例到localStorage
                            const generatedTestCases = testCasesResult.test_cases.map((tc, idx) => ({
                              id: Date.now() + idx,
                              projectId: selectedProject,
                            ...
                            setTestCases([...testCases, ...generatedTestCases]);
                            localStorage.setItem('testCases', JSON.stringify([...testCases, ...generatedTestCases]));

                            setFullPipelineResult(partitionResult);
                            setShowResultModal(true);
                            handleAiGenerateClose();
                            alert(`✅ 成功生成${generatedTestCases.length}个测试用例！`);
                ...
                <table style={styles.paramsTable}>
                  <thead>
                    <tr style={styles.tableHeader}>
                      <th style={{...styles.tableCell, width: '60px'}}>序号</th>
                      <th style={{...styles.tableCell, width: '150px'}}>用例名称</th>
                      <th style={{...styles.tableCell}}>描述</th>
                      <th style={{...styles.tableCell, width: '100px'}}>预期状态</th>
                      <th style={{...styles.tableCell, width: '60px'}}>操作</th>
                    </tr>
                  </thead>
                  <tbody>
                    {currentProjectCases.filter(c => c.apiId === selectedApi.id).length === 0 ? (
                      <tr>
                        <td colSpan="5" style={{...styles.tableCell, textAlign: 'center', color: '#999'}}>
                          暂无测试用例
                        </td>
                      </tr>
                    ) : (
                      currentProjectCases
                        ...
                        .map((testCase, index) => (
                          <tr key={testCase.id} style={styles.tableRow}>
                            <td style={{...styles.tableCell, width: '60px'}}>{index + 1}</td>
                            <td style={{...styles.tableCell, width: '150px'}}>{testCase.title}</td>
                            <td style={styles.tableCell}>{testCase.description}</td>
                            <td style={{...styles.tableCell, width: '100px'}}>{testCase.expectedStatus}</td>
                            <td style={{...styles.tableCell, width: '60px', whiteSpace: 'nowrap'}}>
                              <button
                                onClick={() => {
